      ...
      <tr v-for="classBehave in classBehavesBak">
        ...
        <td>
          ...
          <div class="hidden-sm hidden-xs btn-group">
            <button v-on:click="edit(classBehave)" class="btn btn-xs btn-info">
              <i class="ace-icon fa fa-pencil bigger-120"></i>
            </button>

            <button v-on:click="del(classBehave.id)" class="btn btn-xs btn-danger">
...
export default {
  ...
  methods: {
    ...
    edit(classBehave) {
      //这个add的作用就是点开那个模态框，新增是save
      let _this = this;
      _this.classBehave = $.extend({}, classBehave);
      $("#form-modal").modal('show');
    },
    list(page) {
      let _this = this;
      Loading.show();
      _this.$ajax.post(process.env.VUE_APP_SERVER + "/business/admin/classBehave/list",
          {
            page: page,
    ...
    save() {
      ...
      _this.$ajax.post(process.env.VUE_APP_SERVER + "/business/admin/classBehave/save",
          _this.classBehave).then((response) => {
        console.log("查询课堂表现列表结果", response);
        let resp = response.data;
        if (resp.success) {
    ...
    del(id) {
      let _this = this;
      Confirm.show("删除课堂表现后不可恢复!",function () {
        Loading.show();
        _this.$ajax.post(process.env.VUE_APP_SERVER + "/business/admin/classBehave/delete/" + id).then((response) => {
          console.log("删除课堂表现列表结果", response);
          let resp = response.data;
          if (resp.success) {
            Toast.success("删除成功");
            _this.list(1);
